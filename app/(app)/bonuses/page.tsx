...
import { bonusCategories, bonusLeaders, bonusPredictionForCategory } from "@/lib/mock";
import { Card, DataFreshnessNote, EmptyState, Pill, SectionHeader } from "../../_components/ui";
      ...
      {bonusCategories.map((category) => {
        ...
        return (
          <Card key={category.id} className="flex flex-col gap-3">
            ...
            {isTerminal && <DataFreshnessNote message={content.terminalPendingHint} />}
            ...
            <div className="flex flex-col gap-2">
              ...
              <ul className="flex flex-col gap-1.5">
                {category.slots.map((slot) => {
                  const pick = prediction?.picks.find((p) => p.slotId === slot.id);
                  return (
                    <li
                      key={slot.id}
                    ...
                    >
                      ...
                      <span className="ltr shrink-0 font-bold tabular-nums text-[var(--gold)]">
            ...
            {leaders && leaders.length > 0 && (
              ...
              >
                ...
                <ul className="flex flex-col gap-1">
                  {leaders.map((leader) => (
                    <li key={leader.rank} className="flex items-center justify-between text-xs">
